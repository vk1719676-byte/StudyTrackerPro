...
<div className="fixed top-4 right-4 z-50"> <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 p-3 min-w-[240px]"> <div className="flex items-center justify-between mb-2"> <div className="flex items-center gap-2"> <div className={`p-1 rounded ${(mode === 'focus' || mode === 'custom') ? 'bg-blue-100 dark:bg-blue-900/30' : 'bg-green-100 dark:bg-green-900/30'}`}> {(mode === 'focus' || mode === 'custom') ? ( <Target className="w-4 h-4 text-blue-600 dark:text-blue-400" /> ) : ( <Coffee className="w-4 h-4 text-green-600 dark:text-green-400" /> )} </div> <span className="text-sm font-medium text-gray-900 dark:text-gray-100"> {mode === 'focus' ? 'Focus' : mode === 'custom' ? 'Custom' : mode === 'shortBreak' ? 'Short Break' : 'Long Break'} </span> </div> <div className="flex gap-1"> <button onClick={handleMaximize} className="p-1 hover:bg-gray-100 dark:hover:bg-gray-700 rounded transition-colors" title="Maximize" > <Maximize2 className="w-3 h-3 text-gray-500" /> </button> <button onClick={handleFloatingStop} className="p-1 hover:bg-gray-100 dark:hover:bg-gray-700 rounded transition-colors" title="Stop Timer" > <Square className="w-3 h-3 text-red-500" /> </button> </div> </div>
