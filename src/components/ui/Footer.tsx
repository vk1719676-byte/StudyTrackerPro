...
export const Footer: React.FC = () => {
  ...
  // AI Question Solver States
  const [showAIQuestionSolver, setShowAIQuestionSolver] = useState(false);
  const [aiQuestion, setAIQuestion] = useState('');
  const [aiAnswer, setAIAnswer] = useState('');
  const [aiSteps, setAISteps] = useState<string[]>([]);
  const [aiSubject, setAISubject] = useState('mathematics');
  const [aiQuestionType, setAIQuestionType] = useState('problem-solving');
  ...
  const [aiRelatedTopics, setAIRelatedTopics] = useState<string[]>([]);
  const [aiImageUpload, setAIImageUpload] = useState<File | null>(null);
  ...
  const handleFeatureClick = (featureName: string) => {
    ...
    if (featureName === 'AI Question Solver') {
      setShowAIQuestionSolver(true);
      return;
    }
  ...
  const studyTools = [
    {
      ...
      description: "Advanced GPT-powered AI that solves any study question with step-by-step explanations, image recognition, and voice input.",
  ...
  // Advanced AI Question Solver Functions
  const solveAIQuestion = async () => {
    ...
    try {
      ...
      // Advanced AI Question Processing based on subject and type
      if (aiSubject === 'mathematics') {
        if (aiQuestionType === 'problem-solving') {
          // Advanced mathematical problem solving
          if (aiQuestion.toLowerCase().includes('derivative') || aiQuestion.toLowerCase().includes('calculus')) {
            answer = `To solve this calculus problem, we need to apply differentiation rules systematically.`;
            steps = [
              "Identify the function type and applicable differentiation rules",
              "Apply the power rule: d/dx(x^n) = n·x^(n-1)",
              "Use chain rule if composite functions are present",
              "Simplify the final expression",
              "Verify the result by checking special cases"
            ];
            sources = ["Khan Academy Calculus", "MIT OpenCourseWare", "Paul's Online Math Notes"];
            studyPlan = [
              "Review basic differentiation rules",
              "Practice power rule problems",
              "Study chain rule applications",
              "Work on composite function derivatives"
            ];
            relatedTopics = ["Integration", "Limits", "Chain Rule", "Product Rule", "Quotient Rule"];
            followUpQuestions = [
              "What is the integral of this derivative?",
              "How does the chain rule apply here?",
              "What are the critical points of this function?"
            ];
          } else if (aiQuestion.toLowerCase().includes('algebra') || aiQuestion.toLowerCase().includes('equation')) {
            answer = `This algebraic equation can be solved using systematic algebraic manipulation.`;
            steps = [
              "Identify the type of equation (linear, quadratic, polynomial)",
              "Move all terms to one side if necessary",
              "Factor or use appropriate solving method",
              "Solve for the variable",
              "Check the solution by substitution"
            ];
            sources = ["Algebra.com", "Khan Academy Algebra", "Wolfram Alpha"];
            studyPlan = [
            ...
            relatedTopics = ["Factoring", "Quadratic Formula", "Linear Equations", "Polynomials"];
            followUpQuestions = [
              "What if this equation had multiple solutions?",
              "How would you graph this equation?",
              "What's the domain and range of this function?"
            ];
          } else {
            // General math problem
            answer = `Let me break down this mathematical problem step by step.`;
            steps = [
            ...
            sources = ["Khan Academy", "Coursera Mathematics", "edX Math Courses"];
            confidence = 80;
          }
        } else if (aiQuestionType === 'concept-explanation') {
          answer = `Let me explain this mathematical concept in detail with examples.`;
          steps = [
          ...
          sources = ["Khan Academy", "3Blue1Brown", "Professor Leonard"];
          studyPlan = [
            "Study the fundamental definition",
            "Work through basic examples",
            "Practice application problems",
            "Explore advanced applications"
          ];
        }
      } else if (aiSubject === 'physics') {
        answer = `This physics problem requires understanding of fundamental principles and their applications.`;
        steps = [
          "Identify the physical principles involved",
          "List known and unknown variables",
          "Choose appropriate equations or laws",
          "Solve systematically with units",
          "Check if the result is physically reasonable"
        ];
        sources = ["Khan Academy Physics", "MIT Physics", "Feynman Lectures"];
        studyPlan = [
          "Review fundamental physics principles",
          "Practice problem-solving techniques",
          "Study unit conversions",
          "Work on conceptual understanding"
        ];
        relatedTopics = ["Newton's Laws", "Energy Conservation", "Momentum", "Thermodynamics"];
        followUpQuestions = [
        ...
        answer = `This chemistry question involves understanding molecular behavior and chemical reactions.`;
        steps = [
          "Identify the chemical species and reactions involved",
          "Balance chemical equations if necessary",
          "Apply relevant chemical principles",
          "Calculate using appropriate formulas",
          "Interpret results in chemical context"
        ];
        sources = ["Khan Academy Chemistry", "Crash Course Chemistry", "ChemLibreTexts"];
        studyPlan = [
        ...
        relatedTopics = ["Chemical Bonding", "Stoichiometry", "Thermochemistry", "Kinetics"];
      } else if (aiSubject === 'biology') {
        answer = `This biology question explores the fascinating world of living organisms and their processes.`;
        steps = [
          "Identify the biological system or process",
          "Understand the cellular or molecular mechanisms",
          "Consider the evolutionary context",
          "Analyze the physiological implications",
          "Connect to broader biological principles"
        ];
        sources = ["Khan Academy Biology", "Crash Course Biology", "Nature Education"];
        studyPlan = [
          "Study cell structure and function",
          "Understand DNA and protein synthesis",
          "Learn metabolic pathways",
          "Explore evolutionary biology"
        ];
        relatedTopics = ["Cell Biology", "Genetics", "Evolution", "Ecology", "Physiology"];
      } else if (aiSubject === 'computer-science') {
        answer = `This computer science problem requires algorithmic thinking and programming concepts.`;
        steps = [
        ...
        sources = ["LeetCode", "GeeksforGeeks", "MIT CS Course", "Coursera Algorithms"];
        studyPlan = [
        ...
        relatedTopics = ["Data Structures", "Algorithms", "Time Complexity", "Programming"];
      }

      // Add question-specific customization based on difficulty
      if (aiDifficulty === 'beginner') {
        confidence = Math.max(confidence - 10, 70);
        steps = steps.map(step => `🔰 ${step} (beginner level)`);
        studyPlan.unshift("Start with fundamental concepts");
      } else if (aiDifficulty === 'advanced') {
        confidence = Math.min(confidence + 10, 95);
        steps.push("Explore advanced applications and edge cases");
        studyPlan.push("Study advanced research papers in this area");
      }
      ...
      setAIRelatedTopics(relatedTopics);
      setAIFollowUpQuestions(followUpQuestions);
      ...
      setAIHistory(prev => [questionData, ...prev.slice(0, 9)]); // Keep only last 10

    } catch (error) {
      setAIAnswer('I apologize, but I encountered an error while processing your question. Please try again or rephrase your question.');
      setAISteps(['Error occurred during processing']);
      setAIConfidence(0);
    } finally {
  ...
  const clearAIQuestion = () => {
    ...
    setAIRelatedTopics([]);
    setAIFollowUpQuestions([]);
    setAIImageUpload(null);
  };

  const copyAIAnswer = () => {
    const fullAnswer = `Question: ${aiQuestion}\n\nAnswer: ${aiAnswer}\n\nStep-by-step Solution:\n${aiSteps.map((step, i) => `${i + 1}. ${step}`).join('\n')}`;
    navigator.clipboard.writeText(fullAnswer);
  };

  const saveAIQuestion = () => {
    const questionData = {
      question: aiQuestion,
      answer: aiAnswer,
      steps: aiSteps,
    ...
    const existingSaved = JSON.parse(localStorage.getItem('aiQuestionsSaved') || '[]');
    existingSaved.unshift(questionData);
    localStorage.setItem('aiQuestionsSaved', JSON.stringify(existingSaved.slice(0, 20))); // Keep only 20
    
    alert('Question saved successfully!');
  };

  const rateAIAnswer = (rating: 'good' | 'bad', questionId: string) => {
    setAIHistory(prev => prev.map(item => 
      item.id === questionId ? { ...item, rating } : item
    ));
  };
  ...
  const handleImageUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      setAIImageUpload(file);
      setAIQuestion(`[Image uploaded: ${file.name}] Please analyze this image and solve any problems shown.`);
    }
  };

  const toggleVoiceRecording = () => {
    setAIVoiceRecording(!aiVoiceRecording);
    if (!aiVoiceRecording) {
      // Simulate voice recording
      setTimeout(() => {
        setAIQuestion("Voice input: Explain the concept of derivatives in calculus");
  ...
  return (
    <>
      ...
      {/* AI Question Solver Modal */}
      {showAIQuestionSolver && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-2 sm:p-4">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-6xl max-h-[95vh] overflow-y-auto transform transition-all duration-300 scale-100">
            <div className="relative p-4 sm:p-6">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400">
                      Advanced GPT-powered study assistant with multimodal support
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => setAIShowHistory(!aiShowHistory)}
                  ...
                  <button
                    onClick={() => setShowAIQuestionSolver(false)}
                    className="p-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors duration-200 touch-manipulation"
                  >
                    <X className="w-5 h-5 text-gray-500 dark:text-gray-400" />
                  </button>
              ...
              {/* Configuration Section - Responsive */}
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-purple-500 focus:border-transparent touch-manipulation"
                  >
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="theorem-proof">Theorem & Proofs</option>
                  </select>
                </div>
              ...
              {/* Input Section with Multimodal Support - Responsive */}
              <div className="mb-6">
                ...
                <div className="flex flex-wrap gap-2 mb-3">
                  <label className="flex items-center gap-2 px-3 py-2 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 rounded-lg cursor-pointer hover:bg-blue-200 dark:hover:bg-blue-900/50 transition-colors duration-200 text-sm touch-manipulation">
                  ...
                  <button
                    ...
                    className={`flex items-center gap-2 px-3 py-2 rounded-lg transition-colors duration-200 text-sm touch-manipulation ${
                      aiVoiceRecording 
                  ...
                  {aiImageUpload && (
                    <div className="flex items-center gap-2 px-3 py-2 bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 rounded-lg text-sm">
                      <Image className="w-4 h-4" />
                      {aiImageUpload.name}
                      <button
                        onClick={() => setAIImageUpload(null)}
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                  <textarea
                    ...
                    placeholder="Type your question here... (e.g., 'Explain the concept of derivatives in calculus', 'Solve: 2x + 5 = 15', 'What is photosynthesis?')"
                    className="flex-1 min-h-[100px] px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-purple-500 focus:border-transparent resize-vertical touch-manipulation"
                    onKeyDown={(e) => {
                  ...
                  <div className="flex sm:flex-col gap-2">
                    <button
                      onClick={solveAIQuestion}
                      disabled={!aiQuestion.trim() || aiIsLoading}
                      className="flex-1 sm:flex-none px-6 py-3 bg-gradient-to-r from-purple-500 to-pink-600 text-white rounded-lg hover:shadow-lg transition-all duration-300 transform hover:scale-105 disabled:opacity-50 disabled:transform-none flex items-center justify-center gap-2 font-medium touch-manipulation"
                    >
                      {aiIsLoading ? (
                        <>
                    ...
                    <button
                      onClick={clearAIQuestion}
                      className="flex-1 sm:flex-none px-4 py-3 bg-gray-200 dark:bg-gray-600 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-500 transition-colors duration-200 touch-manipulation"
                    >
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">
                  💡 Tip: Press Ctrl+Enter to solve quickly. Supports text, images, and voice input.
                </p>
              </div>

              {/* AI Response Section */}
              {(aiAnswer || aiIsLoading) && (
                <div className="bg-gradient-to-r from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20 rounded-xl p-4 sm:p-6 mb-6 border border-purple-200 dark:border-purple-700">
                  {/* Response Header */}
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
                    ...
                    {aiAnswer && !aiIsLoading && (
                      <div className="flex flex-wrap items-center gap-2">
                        {/* Confidence Score */}
                        <div className="flex items-center gap-2 px-3 py-1 bg-white dark:bg-gray-800 rounded-full border border-purple-200 dark:border-purple-600">
                          <div className={`w-2 h-2 rounded-full ${
                            aiConfidence >= 90 ? 'bg-green-500' :
                            aiConfidence >= 70 ? 'bg-yellow-500' : 'bg-red-500'
                          }`}></div>
                        ...
                        <button
                          ...
                          className="p-2 bg-white dark:bg-gray-800 text-purple-600 dark:text-purple-400 rounded-lg hover:bg-purple-100 dark:hover:bg-purple-900/30 transition-colors duration-200 touch-manipulation"
                          title="Copy Answer"
                        >
                  ...
                  {aiIsLoading ? (
                    <div className="text-center py-8">
                      ...
                      <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                        This may take a few moments for complex problems
                      </p>
                    </div>
                  ) : (
                    <>
                          ...
                          <div className="space-y-3">
                            {aiSteps.map((step, index) => (
                              <div key={index} className="flex items-start gap-3">
                                <div className="w-6 h-6 bg-gradient-to-r from-purple-500 to-pink-600 text-white rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0">
                                  {index + 1}
                                </div>
                      ...
                      {aiRelatedTopics.length > 0 && (
                        <div className="bg-white dark:bg-gray-800 rounded-lg p-4 mb-4 border border-purple-200 dark:border-purple-600">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {aiRelatedTopics.map((topic, index) => (
                              <span
                                key={index}
                                className="px-3 py-1 bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 rounded-full text-sm font-medium cursor-pointer hover:bg-purple-200 dark:hover:bg-purple-900/50 transition-colors duration-200 touch-manipulation"
                                onClick={() => setAIQuestion(`Explain ${topic} in ${aiSubject}`)}
                              >
                                {topic}
                              </span>
                            ))}
                          ...
                          <div className="space-y-2">
                            {aiFollowUpQuestions.map((question, index) => (
                              <button
                                ...
                                className="w-full text-left px-3 py-2 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors duration-200 text-sm touch-manipulation"
                              >
                                {question}
                              </button>
                            ))}
                      ...
                      {/* Rating Section */}
                      <div className="flex items-center justify-center gap-4 pt-4 border-t border-purple-200 dark:border-purple-600">
                        <span className="text-sm text-gray-600 dark:text-gray-400">Was this helpful?</span>
                        <div className="flex gap-2">
                          <button
                            onClick={() => rateAIAnswer('good', Date.now().toString())}
                            className="p-2 text-green-600 hover:bg-green-100 dark:hover:bg-green-900/30 rounded-lg transition-colors duration-200 touch-manipulation"
                            title="Good Answer"
                          >
                            <ThumbsUp className="w-4 h-4" />
                          </button>
                          <button
                            onClick={() => rateAIAnswer('bad', Date.now().toString())}
                            className="p-2 text-red-600 hover:bg-red-100 dark:hover:bg-red-900/30 rounded-lg transition-colors duration-200 touch-manipulation"
                ...
                <div className="bg-gray-50 dark:bg-gray-700 rounded-xl p-4 mb-6">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <button
                      ...
                      className="text-sm text-red-600 dark:text-red-400 hover:underline touch-manipulation"
                    >
                      Clear History
                    </button>
                  </div>
                        ...
                        <div key={item.id} className="bg-white dark:bg-gray-800 rounded-lg p-3 border border-gray-200 dark:border-gray-600">
                          <div className="flex items-start justify-between gap-2 mb-2">
                            ...
                            <div className="flex items-center gap-1 flex-shrink-0">
                              ...
                              {item.rating && (
                                <div className={`w-2 h-2 rounded-full ${
                                  item.rating === 'good' ? 'bg-green-500' : 'bg-red-500'
                                }`}></div>
                              )}
                            </div>
                          </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 text-sm">
                  {[
                    ...
                    <button
                      ...
                      className="text-left px-3 py-2 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-blue-100 dark:hover:bg-blue-900/30 transition-colors duration-200 border border-blue-200 dark:border-blue-700 touch-manipulation"
                    >
                      {example}
                    </button>
                  ))}
              ...
              <div className="bg-gradient-to-r from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20 rounded-xl p-4">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                  ...
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <span className="text-xs font-medium text-gray-700 dark:text-gray-300">Image Analysis</span>
                  </div>
                  <div className="flex flex-col items-center gap-2">
                    <div className="w-10 h-10 bg-gradient-to-r from-green-500 to-teal-600 rounded-full flex items-center justify-center">
                      <Volume2 className="w-5 h-5 text-white" />
                    </div>
                    <span className="text-xs font-medium text-gray-700 dark:text-gray-300">Voice Input</span>
                  </div>
                  <div className="flex flex-col items-center gap-2">
                    <div className="w-10 h-10 bg-gradient-to-r from-orange-500 to-red-500 rounded-full flex items-center justify-center">
                      <BookOpenCheck className="w-5 h-5 text-white" />
                    </div>
                    <span className="text-xs font-medium text-gray-700 dark:text-gray-300">10+ Subjects</span>
                  </div>
                </div>
                <p className="text-xs text-purple-600 dark:text-purple-400 text-center mt-4">
                  🚀 Advanced AI with GPT integration, multimodal support, and comprehensive study assistance
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-6">
              ...
              <p className="text-sm text-gray-600 dark:text-gray-400 max-w-xl mx-auto">
                Powerful AI-enhanced tools to enhance your learning experience
              </p>
            </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-7">
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400 mb-2">
              Your ultimate AI-powered study companion with advanced tools and analytics
            </p>
