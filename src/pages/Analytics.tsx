...
import { PDFReportView } from '../components/analytics/PDFReportView';
...
export const Analytics: React.FC = () => {
  ...
  const handlePDFExport = async () => {
    ...
    try {
      ...
      // Hide the PDF report view after printing
      setTimeout(() => setShowPDFReport(false), 1000);
    }
  };

  // Handle keyboard shortcut for PDF export
  useEffect(() => {
    const handleKeyPress = (e: KeyboardEvent) => {
      if ((e.ctrlKey || e.metaKey) && e.key === 'p' && sessions.length > 0) {
        e.preventDefault();
        handlePDFExport();
      }
    };
  ...
  return (
    <>
      <div className={`min-h-screen bg-gradient-to-br from-violet-50 via-white to-cyan-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 transition-all duration-500 ${showPDFReport ? 'no-print' : ''}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pb-20 md:pb-8 pt-4 md:pt-8">
          ...
          <div className="mb-12 text-center">
            ...
            <div className="flex justify-center mb-8">
              <PDFExportButton onExport={handlePDFExport} loading={exportLoading} />
            </div>
          </div>
      ...
      {/* PDF Report View - Hidden by default, shown when printing */}
      {showPDFReport && (
        <div className={`pdf-report-container ${showPDFReport ? 'active' : ''}`}>
          <PDFReportView
            sessions={sessions}
            exams={exams}
