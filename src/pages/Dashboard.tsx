...
import { BookOpen, Target, TrendingUp, Award, Sparkles, Zap, Star, Calendar, Clock, Trophy, ChevronRight, Brain, Flame, Activity, BarChart3, AlertCircle, CheckCircle2, Timer, X, Lightbulb, Rocket, Plus, ArrowRight, TrendingDown, Users, PieChart, Layers, RefreshCw, Eye, BookmarkCheck } from 'lucide-react';
...
// NEW: Study Pattern Analysis Component
const StudyPatternAnalysis: React.FC<{
  sessions: StudySession[];
}> = ({ sessions }) => {
  // Calculate study patterns
  const getStudyPatterns = () => {
    if (sessions.length === 0) return { hourDistribution: [], subjectPerformance: [], weekdayPatterns: [] };
    
    // Time of day analysis
    const hourDistribution = Array(24).fill(0);
    sessions.forEach(session => {
      const hour = new Date(session.date).getHours();
      hourDistribution[hour] += session.duration;
    });
    
    // Subject performance analysis
    const subjectStats: { [key: string]: { time: number; efficiency: number; sessions: number } } = {};
    sessions.forEach(session => {
      if (!subjectStats[session.subject]) {
        subjectStats[session.subject] = { time: 0, efficiency: 0, sessions: 0 };
      }
      subjectStats[session.subject].time += session.duration;
      subjectStats[session.subject].efficiency += session.efficiency;
    ...
    const subjectPerformance = Object.entries(subjectStats).map(([subject, stats]) => ({
      subject,
      time: stats.time,
      avgEfficiency: stats.efficiency / stats.sessions,
      sessions: stats.sessions
    })).sort((a, b) => b.time - a.time);

    // Weekday patterns
    const weekdayStats = Array(7).fill(0).map(() => ({ time: 0, sessions: 0 }));
    sessions.forEach(session => {
      const day = new Date(session.date).getDay();
      weekdayStats[day].time += session.duration;
      weekdayStats[day].sessions += 1;
    });

    const weekdays = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
    const weekdayPatterns = weekdayStats.map((stats, index) => ({
      day: weekdays[index],
      time: stats.time,
      sessions: stats.sessions
    }));
    
    return { hourDistribution, subjectPerformance, weekdayPatterns };
  };

  const getBestStudyTime = () => {
    const hourDistribution = getStudyPatterns().hourDistribution;
  ...
  return (
    <ModernCard className="p-8 h-full">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Study Pattern Analysis</h2>
          <p className="text-gray-600 dark:text-gray-400">Insights into your learning habits</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        ...
        <div className="bg-gradient-to-br from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20 rounded-3xl p-6 border border-purple-200/50 dark:border-purple-700/50">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h3 className="font-bold text-gray-800 dark:text-gray-200 text-lg">Optimal Study Time</h3>
          </div>
        ...
        <div className="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-3xl p-6 border border-blue-200/50 dark:border-blue-700/50">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h3 className="font-bold text-gray-800 dark:text-gray-200 text-lg">Weekly Pattern</h3>
          </div>
          <div className="space-y-2">
            {patterns.weekdayPatterns.slice(1, 6).map((day, index) => (
                ...
                <div className="flex items-center gap-3">
                  <div className="flex-1 h-2 bg-gray-200 dark:bg-gray-700 rounded-full w-20">
                    <div 
                      ...
                      style={{ width: `${Math.min((day.time / Math.max(...patterns.weekdayPatterns.map(d => d.time))) * 100, 100)}%` }}
                    />
                  </div>
      ...
      <div className="bg-gradient-to-br from-emerald-50 to-teal-50 dark:from-emerald-900/20 dark:to-teal-900/20 rounded-3xl p-6 border border-emerald-200/50 dark:border-emerald-700/50">
        <div className="flex items-center gap-3 mb-6">
          <PieChart className="w-6 h-6 text-emerald-600 dark:text-emerald-400" />
          <h3 className="font-bold text-gray-800 dark:text-gray-200 text-lg">Subject Performance</h3>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {patterns.subjectPerformance.slice(0, 6).map((subject, index) => (
            <div key={subject.subject} className="bg-white/50 dark:bg-gray-800/50 rounded-2xl p-4 backdrop-blur-sm">
              <div className="flex items-center justify-between mb-3">
                <h4 className="font-bold text-gray-800 dark:text-gray-200 truncate">{subject.subject}</h4>
                <div className="flex items-center gap-1">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <div
                      key={star}
                      className={`w-2 h-2 rounded-full ${
                        star <= subject.avgEfficiency ? 'bg-yellow-400' : 'bg-gray-300 dark:bg-gray-600'
                      }`}
                    />
                  ))}
                </div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-bold text-gray-800 dark:text-gray-200">
                    {Math.round(subject.time)}m
                  </span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-600 dark:text-gray-400">Sessions</span>
                  <span className="font-bold text-gray-800 dark:text-gray-200">{subject.sessions}</span>
                </div>
              </div>
            </div>
          ))}
...
// NEW: Knowledge Retention Tracker Component
const KnowledgeRetentionTracker: React.FC<{
  sessions: StudySession[];
  exams: Exam[];
}> = ({ sessions, exams }) => {
  // Calculate retention metrics
  const getRetentionMetrics = () => {
    const subjectRetention: { [key: string]: { sessions: StudySession[]; avgEfficiency: number } } = {};
    
    sessions.forEach(session => {
      if (!subjectRetention[session.subject]) {
        subjectRetention[session.subject] = { sessions: [], avgEfficiency: 0 };
      }
      subjectRetention[session.subject].sessions.push(session);
    });

    Object.keys(subjectRetention).forEach(subject => {
      const subjectSessions = subjectRetention[subject].sessions.sort((a, b) => 
        new Date(a.date).getTime() - new Date(b.date).getTime()
      );
      const totalEfficiency = subjectSessions.reduce((sum, s) => sum + s.efficiency, 0);
      subjectRetention[subject].avgEfficiency = totalEfficiency / subjectSessions.length;
    });
  ...
  const getReviewRecommendations = () => {
    ...
    const subjectLastStudy: { [key: string]: Date } = {};
    sessions.forEach(session => {
      const sessionDate = new Date(session.date);
      if (!subjectLastStudy[session.subject] || sessionDate > subjectLastStudy[session.subject]) {
        subjectLastStudy[session.subject] = sessionDate;
      }
    });

    Object.entries(subjectLastStudy).forEach(([subject, lastDate]) => {
      const daysSince = Math.floor((now.getTime() - lastDate.getTime()) / (1000 * 60 * 60 * 24));
      let priority: 'high' | 'medium' | 'low' = 'low';
      let reason = 'Regular review recommended';

      if (daysSince >= 7) {
        priority = 'high';
        reason = 'Long time since last study';
      } else if (daysSince >= 3) {
        priority = 'medium';
        reason = 'Time for a refresh';
      }
    ...
    return recommendations.sort((a, b) => b.daysSinceLastStudy - a.daysSinceLastStudy);
  };

  const retentionMetrics = getRetentionMetrics();
  const reviewRecommendations = getReviewRecommendations();
  const overallRetention = Object.values(retentionMetrics).length > 0 
    ? Object.values(retentionMetrics).reduce((sum, data) => sum + data.avgEfficiency, 0) / Object.values(retentionMetrics).length 
    : 0;
  ...
  return (
    <ModernCard className="p-8 h-full">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Knowledge Retention</h2>
          <p className="text-gray-600 dark:text-gray-400">Track your learning retention and get review recommendations</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        <div className="lg:col-span-1">
          <div className="bg-gradient-to-br from-emerald-50 to-teal-50 dark:from-emerald-900/20 dark:to-teal-900/20 rounded-3xl p-6 border border-emerald-200/50 dark:border-emerald-700/50 text-center">
            <div className="flex items-center justify-center gap-3 mb-4">
              <BookmarkCheck className="w-8 h-8 text-emerald-600 dark:text-emerald-400" />
              <h3 className="font-bold text-gray-800 dark:text-gray-200 text-xl">Retention Score</h3>
            </div>
            <div className="relative w-32 h-32 mx-auto mb-4">
              ...
              <div 
                ...
                style={{ 
                  borderTopColor: overallRetention >= 4 ? '#10b981' : overallRetention >= 3 ? '#f59e0b' : '#ef4444',
                  borderRightColor: overallRetention >= 4 ? '#10b981' : overallRetention >= 3 ? '#f59e0b' : '#ef4444'
                }}
              ></div>
        ...
        <div className="lg:col-span-2">
          <div className="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-3xl p-6 border border-blue-200/50 dark:border-blue-700/50 h-full">
            <div className="flex items-center gap-3 mb-6">
              <Layers className="w-6 h-6 text-blue-600 dark:text-blue-400" />
              <h3 className="font-bold text-gray-800 dark:text-gray-200 text-lg">Subject Breakdown</h3>
            </div>
            <div className="space-y-4">
              {Object.entries(retentionMetrics).slice(0, 4).map(([subject, data]) => (
                <div key={subject} className="bg-white/50 dark:bg-gray-800/50 rounded-2xl p-4 backdrop-blur-sm">
                  <div className="flex items-center justify-between mb-3">
                    <h4 className="font-bold text-gray-800 dark:text-gray-200">{subject}</h4>
                    <div className={`px-3 py-1 rounded-full text-sm font-bold ${
                      data.avgEfficiency >= 4 
                        ? 'text-emerald-700 bg-emerald-200 dark:text-emerald-300 dark:bg-emerald-800/40'
                        : data.avgEfficiency >= 3
                        ? 'text-blue-700 bg-blue-200 dark:text-blue-300 dark:bg-blue-800/40'
                        : 'text-red-700 bg-red-200 dark:text-red-300 dark:bg-red-800/40'
                    }`}>
                      {data.avgEfficiency.toFixed(1)}/5.0
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    <div className="flex-1 h-3 bg-gray-200 dark:bg-gray-700 rounded-full">
                      <div 
                        ...
                        style={{ width: `${(data.avgEfficiency / 5) * 100}%` }}
                      />
                    </div>
      ...
      <div className="bg-gradient-to-br from-orange-50 to-amber-50 dark:from-orange-900/20 dark:to-amber-900/20 rounded-3xl p-6 border border-orange-200/50 dark:border-orange-700/50">
        <div className="flex items-center gap-3 mb-6">
          <RefreshCw className="w-6 h-6 text-orange-600 dark:text-orange-400" />
          <h3 className="font-bold text-gray-800 dark:text-gray-200 text-lg">Review Recommendations</h3>
        </div>
        
        {reviewRecommendations.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {reviewRecommendations.slice(0, 4).map((rec, index) => (
              <div key={rec.subject} className="bg-white/50 dark:bg-gray-800/50 rounded-2xl p-4 backdrop-blur-sm">
                <div className="flex items-center justify-between mb-3">
                  <h4 className="font-bold text-gray-800 dark:text-gray-200 truncate">{rec.subject}</h4>
                  <div className={`px-3 py-1 rounded-full text-xs font-bold ${
                    rec.priority === 'high' 
                      ? 'text-red-700 bg-red-200 dark:text-red-300 dark:bg-red-800/40'
                      : rec.priority === 'medium'
                      ? 'text-orange-700 bg-orange-200 dark:text-orange-300 dark:bg-orange-800/40'
                      : 'text-blue-700 bg-blue-200 dark:text-blue-300 dark:bg-blue-800/40'
                  }`}>
                ...
                <div className="flex items-center justify-between">
                  <span className="text-xs text-gray-500 dark:text-gray-500">
                    {rec.daysSinceLastStudy} days ago
                  </span>
                  <button className="text-xs bg-orange-500 hover:bg-orange-600 text-white px-3 py-1 rounded-full font-bold transition-colors duration-200">
                    Review Now
                  </button>
                </div>
              </div>
          ...
          <div className="text-center py-8">
            ...
            <h4 className="font-bold text-gray-800 dark:text-gray-200 text-lg mb-2">All Up to Date!</h4>
            <p className="text-gray-600 dark:text-gray-400">No subjects need immediate review. Keep up the great work!</p>
          </div>
        )}
      </div>
    </ModernCard>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-indigo-50/30 dark:from-gray-900 dark:via-gray-900 dark:to-gray-800">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-20 md:pt-8 pb-24 md:pb-12">
        ...
        {/* NEW: Study Analysis Tools Section */}
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-8 mb-10">
          <StudyPatternAnalysis sessions={sessions} />
          <KnowledgeRetentionTracker sessions={sessions} exams={exams} />
        </div>
